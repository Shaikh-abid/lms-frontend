...
const CourseProgressPage = () => {
  ...
  useEffect(() => {
    if (progress?.progress === 100 && !showCompletionModal) {
      setShowConfetti(true);
      setShowCompletionModal(true);
      setTimeout(() => setShowConfetti(false), 5000);
    }
  }, [progress?.progress, showCompletionModal]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-card rounded-3xl p-8 max-w-md mx-4 text-center space-y-6 shadow-2xl animate-scale-in">
            ...
            <div className="space-y-3">
              <Button variant="gradient" className="w-full" onClick={() => setShowCompletionModal(false)}>
                Continue Learning
              </Button>
              <Button variant="outline" className="w-full" onClick={() => navigate('/my-courses')}>
                Back to My Courses
              </Button>
            </div>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Video Section */}
        <div className={cn("flex-1 flex flex-col", isSidebarOpen && "hidden lg:flex")}>
          {/* Video Player */}
          <div className="bg-foreground aspect-video w-full">
            {currentLecture ? (
              <video
                ref={videoRef}
